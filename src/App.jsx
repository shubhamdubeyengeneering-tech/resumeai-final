...
function WorkspaceModules({ page, currentUser, onNavigate, resumeJobId, onLogout }) {
  ...
  if (page === "profile") return <SimplePage title="Profile" icon="👤" description="Build the professional identity ResumeAI uses across your career workspace.">
    <div className="professional-module profile-command-center animated-module">
      <div className="profile-hero-pro"><div className="profile-avatar-xl profile-avatar-gradient">{(safeProfile.name || "U").slice(0,1).toUpperCase()}</div><div className="profile-hero-copy"><span className="eyebrow">CAREER IDENTITY</span><h2>{safeProfile.name || "Your Name"} <span className="verified-dot">✓</span></h2><p>{safeProfile.headline || "Add a professional headline to make your profile stronger."}</p><div className="profile-meta"><span>✉️ {safeProfile.email || "Email not added"}</span><span>📍 {safeProfile.location || "Location not added"}</span></div></div><div className="profile-completion-ring"><strong>{Math.min(100, Math.round(([safeProfile.name,safeProfile.email,safeProfile.phone,safeProfile.location,safeProfile.headline,safeProfile.bio,safeProfile.skills].filter(Boolean).length / 7) * 100))}%</strong><small>Complete</small></div></div>
      ...
      <form className="profile-editor-pro" onSubmit={saveProfile}><div className="profile-editor-title"><span>✨</span><div><h3>Professional profile</h3><p>These details stay connected to your ResumeAI workspace.</p></div></div><div className="profile-field-grid"><label>Full Name<input value={safeProfile.name} onChange={e=>setProfile({...profile,name:e.target.value})} placeholder="Your name" required/></label><label>Email Address<input value={safeProfile.email} readOnly/></label><label>Phone Number<input value={safeProfile.phone} onChange={e=>setProfile({...profile,phone:e.target.value})} placeholder="+91 ..."/></label><label>Location<input value={safeProfile.location} onChange={e=>setProfile({...profile,location:e.target.value})} placeholder="City, Country"/></label><label className="full">Professional Headline<input value={safeProfile.headline} onChange={e=>setProfile({...profile,headline:e.target.value})} placeholder="e.g. Software Developer | React & Python"/></label><label className="full">Professional Summary<textarea rows="5" value={safeProfile.bio} onChange={e=>setProfile({...profile,bio:e.target.value})} placeholder="Write a concise introduction based on your real experience..."/></label><label className="full">Skills & Technologies<textarea rows="3" value={safeProfile.skills} onChange={e=>setProfile({...profile,skills:e.target.value})} placeholder="Python, React, FastAPI, SQL..."/></label></div><div className="profile-form-footer full"><span>🔒 Your profile is saved for your workspace.</span><button className="workspace-primary glow-action" disabled={busy}>{busy ? "Saving..." : "Save Profile →"}</button></div></form>{message && <p className="module-message">{message}</p>}
    </div>
  </SimplePage>;
